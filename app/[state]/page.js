...
function CustomDropdown({ options, value, onChange, placeholder, label }) {
 ...
 const metadata = ({ params }) => {
  ...
  const stateName = state
    ...
    .join(' ');
  const url = `https://yourdomain.com/${params?.state || ''}`;
  return {
    title: `Top Solar Companies in ${stateName} | SunSolar`,
    description: `Find the best solar companies in ${stateName} for your residential and commercial needs. Compare installers and get quotes for solar solutions in ${stateName}.`,
    openGraph: {
      title: `Top Solar Companies in ${stateName} | SunSolar`,
      description: `Find the best solar companies in ${stateName} for your residential and commercial needs. Compare installers and get quotes for solar solutions in ${stateName}.`,
      url,
    ...
    <main className="relative bg-gray-900 min-h-screen ">
      <section className="flex flex-col md:flex-row gap-8 max-w-[1700px] mx-auto py-20 px-2 mt-15">
        ...
        <div className="flex-1 pt-10">
          <div className="mb-6">
            <h2 className="text-4xl text-white text-center font-bold mb-2 drop-shadow">Top Solar Companies in {stateName}</h2>
            <p className="text-gray-300 mt-2 mb-4 text-center">Find the best solar companies in {stateName} for your residential and commercial needs.</p>
            <div className="flex items-center justify-center mt-4 w-full">
              <p className="text-sm text-gray-400 text-center mx-auto">
                ...
                {selectedDistrict && (
                  <span className="ml-2 inline-block bg-green-900 text-green-200 px-2 py-1 rounded text-xs ">{selectedDistrict}</span>
                )}
              </p>
            </div>
            {/* Companies List */}
            <div className="space-y-6 mt-8 w-[100%]">
              {filteredCompanies.length === 0 ? (
                ...
                filteredCompanies.map((company) => (
                  <div key={company.slug || company.id} className='w-full lg:w-[70%] h-[220px] flex items-center bg-gradient-to-br from-gray-800 via-gray-900 to-gray-800 shadow-xl rounded-2xl border-l-8 border-pink-500 hover:scale-105 hover:shadow-2xl transition-all duration-300 mx-auto animate-fade-in'>
                    ...
                    <div className='w-[75%] h-full p-7 flex justify-between items-center'>
                      <div className='w-[70%] h-full flex flex-col justify-center'>
                        <h5 className='text-2xl font-bold text-white mb-2 flex items-center gap-2'>
                          {company['company-name'] || company.name}
                          {company.rating && company.rating >= 4.5 && (
                            <span className="ml-2 px-2 py-1 bg-pink-600 text-white text-xs rounded-full">Top Rated</span>
                          )}
                        </h5>
                        <p className='text-gray-300 text-sm mb-4 line-clamp-2'>{company.district || company.location}</p>
                        <a
                      ...
                      <div className='flex flex-col items-center justify-center'>
                        <div className='flex items-center gap-1 mb-1'>
                          ...
                          <span className='text-lg font-bold text-white'>{company.rating || '4.0'}</span>
                        </div>
        ...
        <div className="bg-gray-700 w-[4px] h-[400px] mt-10 rounded-full shadow-md"></div>
        {/* DISTRICT LIST SIDEBAR */}
        <aside className="w-[50%] h-auto md:h-[550px] p-5 mt-10 md:w-1/4 p-8 mb-8 md:mb-0 flex flex-col gap-2">
          <h2 className="text-2xl font-bold text-white mb-6">Districts</h2>
              ...
              <button
                ...
                className={`inline-flex items-center px-4 py-1.5 rounded-full border text-base font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-pink-400 shadow-sm min-w-[140px] whitespace-normal
                  ${selectedDistrict === district
                    ? 'bg-gradient-to-r from-pink-600 to-pink-400 text-white border-pink-400 shadow-lg scale-105'
                    : 'bg-gradient-to-r from-gray-800 to-gray-700 text-gray-200 border-gray-600 hover:bg-pink-700 hover:text-white hover:scale-105'}
                `}
